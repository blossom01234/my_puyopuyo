class PuyoImage {
  static initialize() {
    ...
    for (let i = this.puyoImages.length - 1; i >= 0; i--) {
      ...
      [this.puyoImages[j], this.puyoImages[i]]
    }
  ...
  static batankyu(frame) {
    ...
    const x = Math.cos(Math.PI / 2 + ratio * Math.PI * 2 * 10) *
      Config.puyoImgWidth;
    const y = (Math.cos(Math.PI * ratio * Math.PI * 2) *
      Config.puyoImgHeight *
      Config.stageRows) / 4 +(Config.puyoImgHeight * Config.stageRows) / 2;
